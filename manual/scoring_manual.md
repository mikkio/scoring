...
通っているところにコピーする。
* `bin/score`: scoringパッケージを呼び出すbashシェル。scoringパッケージのすべての機能は
このscoreコマンドを通して提供される。

`make install`で配置したい場合は`Makefile`の中の以下の2つの変数を
正しくセットしてから、`make install`すればよい。
@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@cb::\mycolor
```
...
[^mk]: `-marksheet`オプションが指定されている場合は、採点結果に対して
適用される。
...
* 学類のマークシート読取装置と専用ノートパソコンを接続・起動
    * ノートPCのloginはID=mast_fd, pass=IDと同じ
* 「MarkView」というソフトを起動
* 「シート読取り」を選択
