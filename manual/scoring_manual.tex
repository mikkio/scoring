...
\end{tcolorbox}
オプション名は区別が可能な限り短縮してよい。`\texttt{a}'で始まるオプション以外は
すべて1文字で区別できる。
...
\item \texttt{-gakuruistat meibofile}: 学類毎の\texttt{statistics}

\begin{itemize}
\item \texttt{meibofile}はtwinsからダウンロードした名簿ファイル
...
\item twinsからアップロード用のファイルを\ensuremath{\sim}data\slash upload.csvにダウンロード

\item twinsから名簿ファイルを\ensuremath{\sim}meibo\slash meibo.csvにダウンロード
...
\item マークシートの正解と配点重みを定義したファイルを作成する
...
\begin{itemize}
\item \$ score twins\_upload.csv -t -s -g \ensuremath{\sim}meibo\slash meibo.csv -d -abcd -n \&> result\_analysis.txt
